...
class The_Bootstrap_Gallery_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $title = get_the_title( $instance['post_id'] ) ) {
			printf( '%1$s<a href="%2$s" title="%3$s"" rel="bookmark">%4$s</a>%5$s',
				$before_title,
				esc_url( get_permalink( $instance['post_id'] ) ),
				sprintf( esc_attr__( 'Permalink to %s', 'the-bootstrap' ), strip_tags( $title ) ),
				$title,
				$after_title
			);
		}
				...
				<?php
					foreach ( $gallery_posts as $gallery_post ) :
						printf( '<option value="%1$s"%2$s>%3$s</option>',
							esc_attr( $gallery_post->ID ),
							selected( $instance['post_id'],$gallery_post->ID, false ),
							$gallery_post->post_title
						);
					endforeach;
				?>
			</select>
